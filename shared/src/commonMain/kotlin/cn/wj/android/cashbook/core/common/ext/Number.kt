...
/**
 * 数字转换工具方法（纯 Kotlin 部分）。
 *
 * 注意：返回 `java.math.BigDecimal` 的 `toBigDecimalOrZero` 保留在 `core:common`（Android 侧），
 * 因其唯一非测试消费方 `Migration6To7`（历史 migration）依赖 BigDecimal 算术，且 BigDecimal 不可迁 commonMain。
 */
