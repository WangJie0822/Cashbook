...
class RecordAmountTest {
    ...
    fun income_subtracts_charges() {
        assertEquals(9800L, recordAmount(RecordTypeCategoryEnum.INCOME, 10000, 200, 50))
    }

    @Test
    fun expenditure_adds_charges_minus_concessions() {
        assertEquals(10150L, recordAmount(RecordTypeCategoryEnum.EXPENDITURE, 10000, 200, 50))
    }

    @Test
    fun transfer_follows_non_income_branch() {
        // 关键金丝雀：TRANSFER 走"非收入"分支 = amount + charges - concessions（当支出）
        assertEquals(10150L, recordAmount(RecordTypeCategoryEnum.TRANSFER, 10000, 200, 50))
    }
}
