...
object SolarTermUtil {
     ...
     * 每弧度的角秒数
     */
    private const val SECOND_PER_RAD = 180 * 3600 / Math.PI

    /**
     * 每弧度的角度数
     */
    private const val ANGLE_PER_RAD = 180 / Math.PI
     ...
     * 黄赤交角系数表
     */
    private val H_C_ANGLE_TABLE = doubleArrayOf(
        0.0, 50287.92262, 111.24406,
    ...
    private val DTS = doubleArrayOf(
        -4000.0, 108371.7, -13036.80, 392.000, 0.0000, -500.0, 17201.0,
        -627.82, 16.170, -0.3413, -150.0, 12200.6, -346.41, 5.403, -0.1593,
        150.0, 9113.8, -328.13, -1.647, 0.0377, 500.0, 5707.5, -391.41, 0.915,
        0.3145, 900.0, 2203.4, -283.45, 13.034, -0.1778, 1300.0, 490.1, -57.35,
        2.085, -0.0072, 1600.0, 120.0, -9.81, -1.532, 0.1403, 1700.0, 10.2,
        -0.91, 0.510, -0.0370, 1800.0, 13.4, -0.72, 0.202, -0.0193, 1830.0,
        7.8, -1.81, 0.416, -0.0247, 1860.0, 8.3, -0.13, -0.406, 0.0292, 1880.0,
        -5.4, 0.32, -0.183, 0.0173, 1900.0, -2.3, 2.06, 0.169, -0.0135, 1920.0,
        21.2, 1.69, -0.304, 0.0167, 1940.0, 24.2, 1.22, -0.064, 0.0031, 1960.0,
        33.2, 0.51, 0.231, -0.0109, 1980.0, 51.0, 1.29, -0.026, 0.0032, 2000.0,
        64.7, -1.66, 5.224, -0.2905, 2150.0, 279.4, 732.95, 429.579, 0.0158,
        6000.0
    )
     ...
     * 离心率
     */
    private val GXC_E = doubleArrayOf(
        0.016708634, -0.000042037,
        -0.0000001267
    )
    ...
    private val GXC_P = doubleArrayOf(
        102.93735 / ANGLE_PER_RAD, 1.71946 / ANGLE_PER_RAD,
     ...
     * 光行差常数
     */
    private const val GXC_K = 20.49552 / SECOND_PER_RAD
     ...
     * @return 取整数部分
     */
    private fun doubleFloor(v: Double): Double {
        var v = v
        v = Math.floor(v)
     ...
     */
    private fun rad2mrad(v: Double): Double {
        var v = v
        v = v % (2 * Math.PI)
        return if (v < 0) v + 2 * Math.PI else v
    }
     ...
     * @return 计算UTC与原子时的差
     */
    private fun atomTimeDiff(julian: Double): Double {
     ...
     */
    private fun toJulian(time: Time, UTC: Boolean): Double {
        var y = time.year // 取出年月
        var m = time.month
        var n = 0.0
        if (m <= 2) {
            m += 12.0
        ...
        return if (UTC) n + atomTimeDiff(n - J2000) else n
    }
     ...
     * @param UTC UTC
     */
    private fun setFromJulian(jd: Double, UTC: Boolean): Time {
        var jd = jd
        val time = Time()
        ...
        time.year -= 4716.0
        time.month--
        if (time.month > 12) time.month -= 12.0
     ...
     * @param zb zb
     */
    private fun precession(jd: Double, zb: DoubleArray) {
        var i: Int
        var t = 1.0
        var v = 0.0
        val t1 = jd / 365250
     ...
     * @param zb zb
     */
    private fun addGxc(t: Double, zb: DoubleArray) {
        val t1 = t / 36525
        val t2 = t1 * t1
        val t3 = t2 * t1
        val t4 = t3 * t1
        ...
        val dL = L - zb[0]
        val dP = p - zb[0]
        zb[0] -= GXC_K * (Math.cos(dL) - e * Math.cos(dP)) / Math.cos(
            zb[1]
        )
     ...
     * 章动计算
     */
    private val nutB = doubleArrayOf( // 章动表
        2.1824391966, -33.757045954, 0.0000362262, 3.7340E-08, -2.8793E-10,
        -171996.0, -1742.0, 92025.0, 89.0, 3.5069406862, 1256.663930738,
        0.0000105845, 6.9813E-10, -2.2815E-10, -13187.0, -16.0, 5736.0, -31.0,
        1.3375032491, 16799.418221925, -0.0000511866, 6.4626E-08,
        -5.3543E-10, -2274.0, -2.0, 977.0, -5.0, 4.3648783932, -67.514091907,
        0.0000724525, 7.4681E-08, -5.7586E-10, 2062.0, 2.0, -895.0, 5.0,
        0.0431251803, -628.301955171, 0.0000026820, 6.5935E-10, 5.5705E-11,
        -1426.0, 34.0, 54.0, -1.0, 2.3555557435, 8328.691425719, 0.0001545547,
        2.5033E-07, -1.1863E-09, 712.0, 1.0, -7.0, 0.0, 3.4638155059,
        1884.965885909, 0.0000079025, 3.8785E-11, -2.8386E-10, -517.0, 12.0,
        224.0, -6.0, 5.4382493597, 16833.175267879, -0.0000874129, 2.7285E-08,
        -2.4750E-10, -386.0, -4.0, 200.0, 0.0, 3.6930589926, 25128.109647645,
        0.0001033681, 3.1496E-07, -1.7218E-09, -301.0, 0.0, 129.0, -1.0,
     ...
     * @return 计算黄经章动及交角章动
     */
    private fun nutation(t: Double): Nutation {
        var t = t
        val d = Nutation()
        d.Lon = 0.0
        d.Obl = 0.0
     ...
     */
    //地球运动VSOP87参数
    private val E10 = doubleArrayOf( //黄经周期项
        1.75347045673,
        0.00000000000,
        0.0000000000,
        0.03341656456,
    ...
    private val E11 = doubleArrayOf( //黄经泊松1项
        6283.31966747491,
        0.00000000000,
        0.0000000000,
        0.00206058863,
    ...
    private val E12 = doubleArrayOf( //黄经泊松2项
        0.00052918870,
        0.00000000000,
        0.0000000000,
        0.00008719837,
    ...
    private val E20 = doubleArrayOf( //黄纬周期项
        0.00000279620,
        3.19870156017,
        84334.6615813083,
        0.00000101643,
    ...
    private val E30 = doubleArrayOf( //距离周期项
        1.00013988799,
        0.00000000000,
        0.0000000000,
        0.01670699626,
    ...
    private val E32 = doubleArrayOf(0.00004359385, 5.78455133738, 6283.0758499914)
    private val E33 = doubleArrayOf(0.00000144595, 4.27319435148, 6283.0758499914)
     ...
     */
    private fun Enn(F: DoubleArray): Double {
        var v = 0.0
        var i = 0
        while (i < F.size) {
     ...
     */
    private fun earCal(jd: Double): DoubleArray {
        EnnT = jd / 365250
        val llr = DoubleArray(3)
        val t1 = EnnT
        val t2 = t1 * t1
        val t3 = t2 * t1
        val t4 = t3 * t1
        val t5 = (t4
                * t1)
        llr[0] =
            Enn(E10) + Enn(E11) * t1 + Enn(E12) * t2 + Enn(E13) * t3 + Enn(E14) * t4 + Enn(E15) * t5
    ...
    // ==================月位置计算===================
    private var MnnT = 0.0 // 调用Mnn前先设置MnnT时间变量
     ...
     */
    private fun Mnn(F: DoubleArray): Double {
        var v = 0.0
        val t1 = MnnT
        val t2 = t1 * t1
        val t3 = t2 * t1
     ...
     */
    private fun moonCoord(julian: Double): DoubleArray {
        MnnT = julian / 36525
        val t1 = MnnT
        val t2 = t1 * t1
        val t3 = t2 * t1
     ...
     * @return 地心坐标中的日月位置计算
     */
    private fun angleDiff(lx: Int, time: Double, angle: Double): Double {
        val sun = earCal(time) // 计算太阳真位置(先算出日心坐标中地球的位置)
        sun[0] += Math.PI
        sun[1] = -sun[1] // 转为地心坐标
        ...
        val moon = moonCoord(time) // 日月角差与章动无关
    ...
    private fun getTimeFromAngle(t1: Double, angle: Double, lx: Int): Double {
        ...
        var t = 0.0
        var v: Double
        t2 += if (lx == 0) 360.0 // 在t1到t2范围内求解(范气360天范围),结果置于t
        else 25.0
    ...
    fun getSolarTerms(year: Int): Array<String?> {
        val solarTerms = arrayOfNulls<String>(24)
        ...
        System.arraycopy(nextOffset, 0, solarTerms, 22, nextOffset.size)
        val jd = 365.2422 * (year - 2000)
        var q: Double
        for (i in 0..18) {
            q = getTimeFromAngle(jd + i * 15.2, (i * 15).toDouble(), 0)
            q += J2000 + 8.0 / 24 // 计算第i个节气(i=0是春分),结果转为北京时
    ...
    private fun getSolarTermsPreOffset(year: Int): Array<String?> {
        val solarTerms = arrayOfNulls<String>(3)
        val jd = 365.2422 * (year - 2000)
        var q: Double
        for (i in 21..23) {
            q = getTimeFromAngle(jd + i * 15.2, (i * 15).toDouble(), 0)
            q = q + J2000 + 8.0 / 24 // 计算第i个节气(i=0是春分)
            val time = setFromJulian(q, true)
    ...
    private fun getSolarTermsNextOffset(year: Int): Array<String?> {
        val solarTerms = arrayOfNulls<String>(2)
        val jd = 365.2422 * (year - 2000)
        var q: Double
        for (i in 19..20) {
            q = getTimeFromAngle(jd + i * 15.2, (i * 15).toDouble(), 0)
    ...
    private fun doubleToString(value: Double): String {
        val v = value.toInt()
        return if (value < 10) "0$v" else v.toString()
    }

    /**
     * 章动
     */
    internal class Nutation {
        /**
         * 章动角
         */
        internal var Lon = 0.0

        /**
         * 交角
         */
        internal var Obl = 0.0
    }

    internal class Time {
        internal var year = 0.0
        internal var month = 0.0
        internal var day = 0.0
        internal var hour = 0.0
        internal var minute = 0.0
        internal var second = 0.0
