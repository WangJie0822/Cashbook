...
class GetCurrentMonthRecordViewsUseCaseTest {

    @get:Rule
    val dispatcherRule = TestDispatcherRule()

    private lateinit var recordRepository: FakeRecordRepository
    private lateinit var useCase: GetCurrentMonthRecordViewsUseCase
    ...
    fun setup() {
        ...
        useCase = GetCurrentMonthRecordViewsUseCase(
            recordRepository = recordRepository,
    ...
    @Test
    fun when_has_records_then_returns_entities() = runTest {
        recordRepository.addRecord(createRecordModel(id = 1L, typeId = 1L))
        recordRepository.addRecord(createRecordModel(id = 2L, typeId = 1L))

        val result = useCase("2024", "01").first()

        assertThat(result).hasSize(2)
    }

    @Test
    fun when_no_records_then_returns_empty() = runTest {
        val result = useCase("2024", "01").first()
