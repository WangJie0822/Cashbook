...
./gradlew recordRoborazziDebug   # 生成/更新基准截图；本机 record 即权威 reference，CI 当前不 verify/record feature/core 截图（详见「规范（强制）」节 Roborazzi 条款）
...
- **dependabot 依赖升级合并前必查 compileSdk 兼容**：androidx 依赖（core-ktx / lifecycle / activity 等）的某些版本会在 AAR metadata 中声明**消费方最低 compileSdk**。当依赖要求 > 当前 `ProjectSetting.Config.COMPILE_SDK`（现 37）时，AGP `:app:checkXxxAarMetadata` 任务编译期直接 fail（`> N issues were found when checking AAR metadata: Dependency 'X' requires libraries and applications that depend on it to compile against version Y or later`）。**合并 dependabot PR 前必看 CI build 是否绿**，红的若是 `checkAarMetadata` 失败说明依赖要求更高 compileSdk，不可直接合（也不可 `--admin` 绕，会编译阻断 main）。实证 2026-06-23：`--admin` 绕红 CI 合入 #485 core-ktx 1.19.0 + #487 lifecycle 2.11.0（均要求 SDK 37），main 编译阻断，回退至 1.18.0 / 2.9.1 修复（PR #490，main `5e4c6456`）。跟进升 SDK 37 需连带升 AGP（当前 8.12.0 最大推荐 36），属大版本升级专项。2026-07-22 更新：AGP 已升 9.3.0（#508）；compileSdk 已升 37（本条款实证的 SDK 37 依赖群 #501/#498/#497 由专项解锁合入，见 `docs/superpowers/specs/2026-07-22-compilesdk-37-upgrade-design.md`；SDK 37 起 platform 包名带 minor 后缀 `platforms;android-37.0`，AGP 9.3.0 MAX_RECOMMENDED=37.1）。
- **dependabot PR 的 CI 异常处置口径**（2026-07-22 批量处置 11 PR 实操沉淀）：① `gh pr checks` 报 **no checks ≠ CI 未配置**——CI auto-baseline 二次 commit 的 run 卡 `action_required`（需 approve：`gh api repos/:owner/:repo/actions/runs/<id>/approve -X POST`），首 commit 的 run 结果在 `gh run list --branch <pr分支>` 查；② **`@dependabot rebase` 会因 auto-baseline commit 被拒**（"edited by someone other than Dependabot"），改用 `@dependabot recreate`（基于最新 main 重建+自动触发新 CI）；③ **CI 失败先辨环境假失败**——aliyun 镜像海外访问间歇 502/404（`Could not GET ... 502` / `Could not find ...jar`）是假失败，但**假失败可能叠在真问题之上**（#501 两次 502 掩盖 compileSdk 37 硬冲突），修好环境必须重验干净 run 才可合；④ 失败 run 保存的 actions cache 含 **Gradle 负缓存**（artifact missing 记录），环境恢复后 rerun 仍假败，须 `gh cache delete` 清 `gradle-dependencies-v1-*`（+ 要真跑测试时连 `gradle-build-cache-v1-*`）再 rerun；⑤ 多 PR 连续合入前用 `gh pr diff <n> --name-only` 查 baseline 冲突面——仅动 runtime classpath 的升级（compose 等）带 `dependencies/*.txt` 变化，AGP/baselineprofile 等 buildscript classpath 升级不动 baseline，无交叉可连续合；⑥ 分支保护 `required_status_checks=null` 时 CI 非合并门禁，`--admin` merge 前必须自查 CI 结论。
- **依赖回退/baseline 修复必须走 PR 不直接 push main**：`.github/workflows/Build.yaml` 的 Dependency Guard **仅在 `event_name == 'pull_request'` 时**自动 `dependencyGuardBaseline` 并 auto-commit 回 PR 分支（`Push new Dependency Guard baselines` step）。（⚠️ 同 workflow 里的 Roborazzi `recordRoborazziDevDebug` 是 app flavor task、Gradle 无 `:` 前缀只匹配 app，**不覆盖 feature/core 截图**，并非「PR 自动回填 feature 基线」的机制——详见「规范（强制）」节 Roborazzi 条款。）直接 push 到 main 时 fork 检查路径会 `exit 1`，baseline 不会自动重生成。故依赖版本变更（包括回退）必须**开 PR**让 CI 自动回填 baseline，再 `--admin` merge（review_required 单人 owner 无法自我 approve）。
...
- **截图基线 record 与判失败方法论**：① 判 `verifyRoborazzi` 失败**只信本次 run 的 `testDebugUnitTest` FAILED 标记**（gradle 输出）+ `build/outputs/roborazzi/*_compare.png` 的 **mtime**，**勿被上次 run 残留的 stale compare 图误导**（曾据残留误判「settings 17 + records 1 截图失败」，核查 b-verify3 日志实际仅 assets FAILED、re-record 后 `git status` 0 变化反证基线本就正确，main 2026-06-25）。② 缺失基线判据：`git ls-files <module>/src/test/screenshots/ | wc -l` = 0 即测试代码入库但基线**从未 record**（本地 verify 必失败、Reference 空白；**CI 当前不 verify/record feature/core 截图（app flavor task 不覆盖库模块，见下条），缺失基线不会被 CI 暴露也不会被 CI 补录，须本地首录**）→ `recordRoborazziDebug` 生成 → **record 前先 grep `\.now()` 排查时间脆弱性** → 视觉抽检 New 非塌陷（`android screen capture` 对 Compose 恒全白，靠 Read 基线 PNG）→ `verifyRoborazzi` 0 diff 确认确定性 → 入库。实证 2026-06-25 补齐 assets/core:ui/tags/books 缺失基线 178 张（main `9db577aa`）。③ **全量 verify 批量 FAIL 判别与重录口径（2026-07-24 更正版：原 2026-07-23 口径中「机制 P 跨类渲染历史依赖/轨迹绑定」经根查实证不存在并作废，证据链见 `docs/testing/reports/2026-07-23-screenshot-suite-pollution-investigation.md` §9）**：固化 FAIL 集必须 `verifyRoborazziDebug --rerun-tasks --continue`（无 `--continue` 时单 worker 首败即停、并行时部分模块 task 未调度，产生「恰好一致的截断 FAIL 集」；判某模块 0 失败前核对其 test-results XML mtime 落在本次 run 窗口）。**渲染 = 确定性函数(代码+依赖)，与执行轨迹无关**（2026-07-24 五组对照实证：单类/整模块/record/verify/fresh worktree/主 checkout/daemon 形态下渲染逐字节一致；原「MyTags 单类==旧基线、套件 3.48% diff」系 record2 覆盖工作区基线后未还原、verify 对错误参照 PASS 的实验缺陷——同参照系下单类与套件失败集逐用例一致），推论三条：⑴ **单类 verify 红即真信号，可直接用于回归诊断与局部化，无须回落整模块复核**（操作陷阱不变：`--tests` 配 `verifyRoborazziDebug` task 名**不生效**、0 执行假绿——单类须用 `:feature:X:testDebugUnitTest --tests "..." -Proborazzi.test.verify=true`，判真执行只信 `build/test-results/**/*.xml`）；⑵ 增删/重命名截图测试**不影响**其他测试的渲染，下游未改动测试 verify 红不是「轨迹漂移」、须按真变更排查；**record 会直接覆盖 `src/test/screenshots/` 工作区基线，record 后再 verify 前必须 `git status` 核基线状态并显式还原**（本案缺陷根源）；verify PASS 只证「渲染==当前工作区基线」，与任何 git 版本的一致性断言必须落显式字节比对（PASS 不产 `_actual.png`）；⑶ 依赖升级（compose-bom/AGP/Robolectric 等）后全量 verify 大面积红为预期，按报告方法定性（record ×2 验确定性 → 逐像素分级 → 最大通道差 → 目检可见级样本）后全量重录，禁逐张豁免或免责。附注：Roborazzi 比较器对亚感知微差有内建吸收（新旧基线设备帧 ~50px/0.01% 差 verify 绿、3.48% 红，2026-07-24 实测）——**verify 绿 ≠ 字节级一致**。实证 2026-07-23：118 FAIL 全为真实像素差（无 IDENTICAL-yet-FAIL、无加载异常栈），531/676 张漂移（502 张亚感知[含 12 张大面积整偏 delta≤2，其余按像素占比分级+抽检定性] + 29 张 dark_dynamic 可见变化[Material3 动态暗色上游改版，非本项目代码回归]）全量重录 `51e76163`，重录后连续 2 次全量 verify 0 FAIL，118 FAIL 全归因机制 D（依赖漂移）；**敏感性变异实证**：MyTagsScreen padding 8→9dp 单处变异 → 4/4 用例红、还原复绿（roundtrip 绿=稳定性、变异红=敏感性，护栏双向自证）。
...
- **Roborazzi 截图基线由本地 record 维护，CI 当前不 verify/不 record feature/core 截图**：CI 的 `verifyRoborazziDevDebug`/`recordRoborazziDevDebug` 是 **app flavor task**，Gradle 无 `:` 前缀只匹配 app（app 无截图测试，空转）；feature/core 是库模块（无 flavor、仅 Debug 变体），CI **不覆盖其截图 verify/record**。故 feature/core 的 676 张基线（feature 420 + core 256）全靠**本地 `recordRoborazziDebug` 录入库维护，本机 record 即权威 reference**（无 CI 重录覆盖）。**新增/改动常驻 UI 改基线时须本地 `recordRoborazziDebug` 重录并入库，不能依赖 CI 补录**（旧认知「本地只提交代码、CI 会重录覆盖」是错的——CI 根本不 record feature 截图，曾致基线未随 UI 更新、缺失基线积压）。`_dynamic` 变体历史噪声已于 2026-07-23 定性收口（亚感知色阶漂移 + Material3 动态暗色上游改版，随 531 张全量重录 `51e76163` 一并解决；2026-07-24 更正：原归因中的「跨类渲染历史效应」经根查证伪不存在，见调查报告 §9）——**现全量 verify 可信，FAIL 即真变更**（依赖未变时=真回归线索；依赖升级期按「截图基线 record 与判失败方法论」③⑶ 定性、大面积红为预期）；因 CI 不 verify feature 截图，本地录入库的即最终版。**本轮起 feature/core 单测已上 CI**（`Run local tests` step 加 `testDebugUnitTest`）。**CI 截图护栏已上线（2026-07-24，PR #513，spec `docs/superpowers/specs/2026-07-24-ci-screenshot-visual-diff-design.md`，独立 workflow `Screenshot.yaml`）**：① **L `screenshot_freshness` 阻塞 lint**——PR 改 `<module>/src/main/**`（*.kt/res/**）但未动同模块 `src/test/screenshots/**` 即红（脚本 `.github/scripts/screenshot_freshness.sh`，9 截图模块），渲染无关改动打 PR label `screenshot-freshness-skip` 豁免（labeled/unlabeled 触发重判）；② **C' `screenshot_diff` 非阻塞视觉 diff 报告**——同 runner 对 merge-base 与 PR HEAD 各 `recordRoborazziDebug --rerun-tasks` 一次、逐字节比对出 changed/added/removed 进 step summary、changed 新旧对上传 artifact（14 天），**永不 fail**（Phase 0 gating 实证 ubuntu record×2 逐字节一致、单次全量 record ≈6min；变异实证 padding 1dp → 恰好 MyTags 全 14 帧 changed 零误报）；dependabot PR 的大面积 changed 是依赖漂移定性材料非噪声。**CI 仍不做基线 verify/record——本机 record 即权威 reference 不变**；旧 app-flavor verify/record/auto-commit 空转链已从 Build.yaml 删除。安全硬约束见 spec §6（保持 `pull_request` 禁 `pull_request_target`/新 job `contents: read`/SHA 经 env 注入禁 `${{ }}` 插值 run）。实证 2026-07-09（PR #506 CI run 28991717231）：feature/core UnitTest 执行=0、Roborazzi task 全 `:app:`。
...
- **CI 聚合 `testDebugUnitTest`（无前缀）跑库模块单测 + `failOnNoDiscoveredTests` 陷阱（2026-07-09）**：CI `Build.yaml` 的 `Run local tests` step 用**无前缀** `testDebugUnitTest` 聚合**所有库模块**单测（app 由 `testOnlineDebug`/`testOfflineDebug` 覆盖，二者不重叠——app 无 exact `testDebugUnitTest` task，缩写仅在无 exact 匹配时触发故不误伤 app）+ `:shared:testAndroidHostTest`（KMP 模块测试 task **不在**无前缀聚合内，须显式列；原 `:core:model:test` 已随 core:model 迁入 shared 移除，JVM 库 `cashbook.jvm.library` 的 test task 名为 `:test`、同样不在聚合内）。此前只跑 `testOnlineDebug testOfflineDebug`（app-flavor task），feature/core 单测执行=0（CI 盲区）。**两个陷阱**：① Gradle 9 `Test.failOnNoDiscoveredTests` 默认 `true`——「test task 非 NO-SOURCE 但发现 0 @Test」的库模块会判 FAILED 阻断 CI。当前仅 `core:testing`（测试基础设施模块、无 src/test、0 @Test）触发（对照 `core:database` 同为 0 测试但其 test task 是 NO-SOURCE 被 skip、不触发；`core:testing` test 源集本身也 NO-SOURCE，非 NO-SOURCE 的是 `testDebugUnitTest` task 本身，确切机制未完全隔离），已在其 `build.gradle.kts` 就地 `tasks.withType<Test>{ failOnNoDiscoveredTests = false }` 关闭（**不用 build-logic 全局关**，保留其他模块「0 测试即报警」安全网）；今后新增「无单测但 test task 非 NO-SOURCE」的库模块撞红时同样就地关闭 + 注释「日后补真单测须删本段」。② 新增**带测试的 JVM 库**（`cashbook.jvm.library`）须在 `Run local tests` 手动追加 `:new-module:test`——无前缀聚合不含 JVM 库，否则其单测在 CI 静默漏跑（与本次修复盲区同源）。⚠️ feature 模块含 Robolectric 单测，CI（JDK21+有网）首次跑会下载 `android-all-instrumented`；首个 PR CI 若 feature 单测红大概率是环境/依赖下载（本机代理离线跑不了）而非逻辑，须看具体错误。
