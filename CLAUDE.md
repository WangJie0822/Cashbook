...
- **截图基线 record 与判失败方法论**：① 判 `verifyRoborazzi` 失败**只信本次 run 的 `testDebugUnitTest` FAILED 标记**（gradle 输出）+ `build/outputs/roborazzi/*_compare.png` 的 **mtime**，**勿被上次 run 残留的 stale compare 图误导**（曾据残留误判「settings 17 + records 1 截图失败」，核查 b-verify3 日志实际仅 assets FAILED、re-record 后 `git status` 0 变化反证基线本就正确，main 2026-06-25）。② 缺失基线判据：`git ls-files <module>/src/test/screenshots/ | wc -l` = 0 即测试代码入库但基线**从未 record**（本地 verify 必失败、Reference 空白；**CI 当前不 verify/record feature/core 截图（app flavor task 不覆盖库模块，见下条），缺失基线不会被 CI 暴露也不会被 CI 补录，须本地首录**）→ `recordRoborazziDebug` 生成 → **record 前先 grep `\.now()` 排查时间脆弱性** → 视觉抽检 New 非塌陷（`android screen capture` 对 Compose 恒全白，靠 Read 基线 PNG）→ `verifyRoborazzi` 0 diff 确认确定性 → 入库。实证 2026-06-25 补齐 assets/core:ui/tags/books 缺失基线 178 张（main `9db577aa`）。③ **全量 verify 批量 FAIL 判别与重录口径（2026-07-23 重订，推翻同日早段「套件级污染」定性，证据链见 `docs/testing/reports/2026-07-23-screenshot-suite-pollution-investigation.md`）**：固化 FAIL 集必须 `verifyRoborazziDebug --rerun-tasks --continue`（无 `--continue` 时单 worker 首败即停、并行时部分模块 task 未调度，产生「恰好一致的截断 FAIL 集」；判某模块 0 失败前核对其 test-results XML mtime 落在本次 run 窗口）。**基线只对「完整模块套件轨迹」有效**——同 JVM 内前序渲染内容影响后续渲染（内容依赖状态；实证 MyTags dynamic 单类渲染与旧基线逐字节一致、套件下 3.48% diff），推论三条：⑴ 单类跑（`:feature:X:testDebugUnitTest --tests "..." -Proborazzi.test.verify=true`；`--tests` 配 `verifyRoborazziDebug` task 名**不生效**、0 执行假绿，判真执行只信 `build/test-results/**/*.xml`）可能因缺前序渲染历史而假红——**假红 ≠ 基线错，但也不得据单类红直接放行或直接判假**：须回落全量/整模块 verify 复核，整模块绿才可判假红、整模块红即真变更；⑵ 增删/重命名截图测试会改变模块内渲染轨迹：**下游未改动测试 verify 红属预期轨迹漂移（非其自身回归）**，须重录该模块基线随代码同 commit，且重录前须按 ⑶ 同法对 diff 分级+目检定性（防同 commit 夹带的真实 UI 回归被盲录吸收）；CI 未 verify 截图期间忘记重录 = 静默陈旧零信号，靠本地纪律兜底（机制 P 根修前回归红不可局部化，见调查报告 §8 backlog）；⑶ 依赖升级（compose-bom/AGP/Robolectric 等）后全量 verify 大面积红为预期，按报告方法定性（record ×2 验确定性 → 逐像素分级 → 最大通道差 → 目检可见级样本）后全量重录，禁逐张豁免或免责。实证 2026-07-23：118 FAIL 全为真实像素差（无 IDENTICAL-yet-FAIL、无加载异常栈），531/676 张漂移（502 张亚感知[含 12 张大面积整偏 delta≤2，其余按像素占比分级+抽检定性] + 29 张 dark_dynamic 可见变化[抽检指向 Material3 动态暗色上游改版，或叠加机制 P，均非本项目代码回归]）全量重录 `51e76163`，重录后连续 2 次全量 verify 0 FAIL；**敏感性变异实证**：MyTagsScreen padding 8→9dp 单处变异 → 4/4 用例红、还原复绿（roundtrip 绿=稳定性、变异红=敏感性，护栏双向自证）。
