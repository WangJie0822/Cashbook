...
- [x] **Step 1: 写失败测试**(追加到 RecordAmountTest 或新建 AnalyticsPieAmountTest)
...
- [x] **Step 2: 运行确认失败** — Run: `:core:model:test --tests "*AnalyticsPieAmountTest*"` Expected: FAIL

- [x] **Step 3: 实现**(追加到 RecordAmount.kt)
...
- [x] **Step 4: 运行确认通过** — Run: `:core:model:test` Expected: PASS
...
- [x] **Step 2: 运行确认通过**(重构前 — 现有代码 TRANSFER 已走 else=amount-charges,金丝雀应先 PASS,固化现状)

Run: `:core:domain:testDebugUnitTest --tests "*AnalyticsPie*"`
Expected: PASS（固化 TRANSFER→amount-charges 现状,作为重构护栏）

- [x] **Step 3: 改两个 Pie UseCase 委托 analyticsPieAmount**
...
- [x] **Step 4: 运行确认仍通过**(行为等价,含 TRANSFER 金丝雀)
...
- [x] **Step 1: 写/补失败测试(以 GetTypeRecordViewsUseCase 为例,其余同构)**
...
- [x] **Step 2: 运行确认失败/或先 PASS 固化**(若测试是新写的等价断言,重构前现状即单条,应 PASS,作护栏)
...
- [x] **Step 1: 补等价测试**(Flow 收集首值,断言批量==单条逐元素)
- [x] **Step 2: 运行(护栏 PASS)** — Run: `:core:domain:testDebugUnitTest --tests "*GetCurrentMonthRecordViews*"`
- [x] **Step 3: 改造** — `mapLatest { list -> list.map { recordModelTransToViewsUseCase(it).asEntity() } }` 改为 `mapLatest { list -> recordModelTransToViewsUseCase(list).map { it.asEntity() } }`（保留 mapLatest 流式语义）
...
- [x] **Step 1: 补等价测试**(含被 filter 过滤的记录,断言批量结果==单条逐元素,filter 语义保持)
- [x] **Step 2: 运行(护栏)** — Run: `:core:domain:testDebugUnitTest --tests "*GetRelatedRecordViews*"`
- [x] **Step 3: 改造** — `.filter { queryRelatedRecordCountById(it.id) <= 0 }.map { recordModelTransToViewsUseCase(it).asEntity() }` 改为：
```kotlin
...
- [x] **Step 2: 运行确认**(可能直接 PASS,纯函数已实现) — Run: `:core:data:testDebugUnitTest --tests "*BillCategoryMatcherTest*"`
- [x] **Step 3: 如有断言不符,调整测试以匹配实现真实行为**(不改生产代码)
- [x] **Step 4: Commit**
```bash
...
- [x] **Step 1: 写测试**(4 阶策略 + extractCardSuffix;卡号后四位用 `0000`/`1234` 虚构值)
...
- [x] **Step 2-4: 运行/调整/Commit**(同 Task 9 模式)
```bash
...
- [x] **Step 1: 改可见性** — `parseDateTime`、`convertToItem` 加 `@VisibleForTesting`(import `androidx.annotation.VisibleForTesting`)并由 `private` 改 `internal`。
- [x] **Step 2: 写测试**(parseDateTime 三格式 + convertToItem 边界)
...
- [x] **Step 3: 运行确认** — Run: `:core:data:testDebugUnitTest --tests "*WechatBillParserTest*"` Expected: PASS
- [x] **Step 4: Commit**
```bash
...
- [x] **Step 2: 实跑判定 XmlPullParser 可行性**
...
- [x] **Step 3: 制作脱敏 fixture(若 Step 2 走端到端)**
...
- [x] **Step 1: 逐一核实 6 个 ImplTest 实际被测对象**
...
- [x] **Step 4: 全量编译相关模块(确认 ViewModel 调用方不破坏)**
...
- [x] **Step 1: 读现状** — Read 现有测试(已覆盖去重 POSSIBLE/NONE),确认 Fake 装配方式。
- [x] **Step 2: 补 happy-path 测试** — 验证导入预览:金额由元(Double)正确转分(Long,`toCent`)、direction→收支方向、去重状态。用 `FakeRecordRepository` 等既有 Fake。
- [x] **Step 3: 运行** — Run: `:feature:record-import:testDebugUnitTest` Expected: PASS
- [x] **Step 4: Commit**
```bash
...
- [x] **Step 1: 加测试依赖** — `sync/work/build.gradle.kts` `dependencies {}` 加:
```kotlin
...
- [x] **Step 3: 运行确认失败** — Run: `:sync:work:testDebugUnitTest` Expected: FAIL(doWork 当前恒 success)
...
- [x] **Step 5: 运行确认通过** — Run: `:sync:work:testDebugUnitTest` Expected: PASS（3 测全绿）
...
- [x] **全量回归**:涉及模块逐个跑 `testDebugUnitTest`(core:model `:test`),全 BUILD SUCCESSFUL。
- [x] **节点 2 full-review**:对 `git diff main..HEAD` 跑 `comprehensive-review:full-review`,修 Critical/High。
- [x] **finishing-branch**:FF 合并回 main。
