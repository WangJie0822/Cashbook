...
>
> **节点2 评审订正（2026-07-09）**：以下 Step 0 注释模板与 commit message 原述「junit/Hilt testing 依赖使 test 源集非 NO-SOURCE」经 code-reviewer 实证**不准**——`core:testing` 的 test 源集本身是 NO-SOURCE（空，`compileDebugUnitTestKotlin` 亦 NO-SOURCE），真正非 NO-SOURCE 的是 `testDebugUnitTest` **task**（确切机制未完全隔离，疑与测试基础设施库的插件/classpath 有关）；`core/testing/build.gradle.kts` **最终注释已订正为事实准确版**。基线数以实测 **676**（feature 420 + core 256）为准（非旧记 728）。commit `14b65d0b` message 措辞未 amend（结论正确、仅机制措辞不精确，rebase 4 commit 不值）。
...
git -C D:/wt/Cashbook/ci-coverage-fix commit -m "[docs|公共|CLAUDE.md截图认知纠正][公共]4 处纠正 CI 管理截图基线的错误认知（CI 不 verify/record feature/core 截图，676 基线本地 record 维护；本轮起单测上 CI，截图 verify 待独立 spec）"
```
