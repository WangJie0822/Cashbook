...
- [x] **Step 2: RecordViewsModel 增字段（默认 NONE）**
...
- [x] **Step 3: RecordViewsEntity 增字段（默认 NONE）**
...
- [x] **Step 4: ModelTransfer 真实传递（H1）**
...
- [x] **Step 3: 改实现（取反 recordAmount，H2）**
...
- [x] **Step 1: 写失败测试（四态，fixture 用固定负 ID — M5）**
...
- [x] **Step 4: 运行验证通过 + 回归（含 batch 等价 + 无 N+1）**
...
- [x] **Step 2: 列表前缀按 relatedNature + 待报销标签**
...
> **实施勘误（2026-06-04）**：① `statusRes` 加 `EXPENDITURE` 守卫（`if (typeCategory == EXPENDITURE) { when(relatedNature){...} } else null`）——避免 INCOME 且 `reimbursable=true` 误显示「待报销」（生产中 reimbursable 仅 EXPENDITURE 可设，但 preview 数据 `reimbursedIncomeRecordViewsData.reimbursable=true` 会暴露，守卫与原逻辑一致更健壮）。② preview 为静态数据不经 domain 计算，**直接设 `relatedNature` 字段**即可（无需改 `relatedRecordList` 的 typeId——那是 domain 单测 fixture 的要求，Task 3 已用固定负 ID 满足）。

- [x] **Step 1: 详情金额行标注按 relatedNature（B）**
...
- [x] **Step 2: 关联记录行改用 relatedAmount（A / M7）**
...
- [x] **Step 3: 补 preview 样例（M5：关联记录 typeId 固定负 ID）**
