...
| `Migration9To10` 加 `final_amount` 列时 `DEFAULT 0 NOT NULL`——未跑应用层填充前全为 0 | `Migration9To10.kt:44` |
...
**两分支差异的事实依据**：`!db9To10DataMigrated` 用户（新装/从 v9 升级）在 `migrateAfter9To10()` 跑完前 `final_amount` 全为 `Migration9To10` 的 DEFAULT **0**，首屏会显示全 `0.00` → 必须 gate；netRecalc 老用户已在吸收模型版本填过有效 finalAmount（仅被吸收报销支出显 0、吸收者偏差），不 gate 显旧值、重算后自动刷新，可接受。
...
init {
    viewModelScope.launch {
        ...
        if (!tempKeys.db9To10DataMigrated) {
            ...
            // 已迁移：立即放行首屏（finalAmount 已是有效值，至多吸收模型旧值）
            _migrationCompleted.value = true
...
- **新增**：netRecalc 分支下首屏立即 `Success`（不卡 Loading）——需让 `FakeRecordRepository.recalculateAllFinalAmount` 支持**可控挂起**（加一个 gate 开关 / CompletableDeferred），在重算"进行中"断言 `uiState` 已是 `Success`，再放行重算确认无异常。
- **新增**：db9To10 分支（`!db9To10DataMigrated`）在 `migrateAfter9To10` 未完成时 `uiState` 为 `Loading`，完成后转 `Success`——同样需 Fake 的 `migrateAfter9To10` 可控挂起。
- **测试替身改造**：`FakeRecordRepository` 给 `recalculateAllFinalAmount` / `migrateAfter9To10` 加可控挂起开关（默认立即返回，保持现有测试不破）。

### `BackupRecoveryManagerImpl`（F2）

- 先核实 `BackupRecoveryManagerImpl` 现有测试覆盖情况（writing-plans 阶段确认）。
- 验证恢复成功路径调用 `recordRepository.recalculateAllFinalAmount()`（而非直调 DAO）。若现有无该 Manager 的单测且实例化成本高（依赖 Context/WebDAV/database），降级为：以构造签名变更 + 调用点替换的编译期保证 + 复用 repository 已有的 `recalculateAllFinalAmount` 单测覆盖（`TransactionDaoLogicTest` / repository 测试）。
...
- **db9To10 分支体验不变**：保留 gate，首屏 Loading 直到 finalAmount 填充完成，避免显示全 0。
